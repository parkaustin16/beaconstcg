...
export default function ForgotPasswordClient() {
  ...
  const handleResetRequest = async (event: React.FormEvent) => {
    ...
    try {
      const redirectTo = `${window.location.origin}${withLang('/auth/reset-password')}`;
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white dark:bg-zinc-800 rounded-lg shadow-md p-8">
        <h1 className="text-2xl font-bold text-zinc-900 dark:text-white text-center">{t.auth.forgotPasswordTitle}</h1>
